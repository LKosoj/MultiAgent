...
class SchemaEnricher:
        ...
        * Retryable (network/timeout): retry выполняется внутри
        ...
        for table_name in all_tables_to_process:
            ...
            try:
                ...
                # Передаём схему явно в get_table_sample_data → _get_fk_previews,
    ...
    def get_table_sample_data(
        ...
        try:
            ...
            try:
                ...
                fk_previews = self._get_fk_previews(conn, plugin, table_name, schema_obj=schema_obj)
    ...
    def _get_fk_previews(
        ...
        if hasattr(plugin, 'get_fk_preview'):
            try:
                # Используем явно переданную схему; self._cached_schema — fallback
                # для устаревших прямых вызовов вне enrich_descriptions_with_llm.
                current_schema = schema_obj if schema_obj is not None else getattr(self, '_cached_schema', {})
