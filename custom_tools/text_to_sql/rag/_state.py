...
class SharedIndexState:
    ...
    def _reset_all(cls) -> None:
        """Сбрасывает class-level state после fork в дочернем процессе.

        Вызывается через ``os.register_at_fork(after_in_child=...)``.
        RLock, скопированный через fork, может быть в заблокированном
        состоянии (если fork произошёл, пока родительский поток держал
        лок). Полная замена объектов новыми — единственный безопасный путь.
        """
        cls._index_registry = {}
        cls._per_session_locks = {}
...
# os.register_at_fork доступен только на Unix (не на Windows).
# Сбрасываем class-level state в дочернем процессе после fork, чтобы
# не наследовать заблокированные RLock'и (M29).
if hasattr(os, "register_at_fork"):
    os.register_at_fork(after_in_child=SharedIndexState._reset_all)
