...
_loader: YamlConfigLoader["SimilarityThresholdsConfig"] = YamlConfigLoader[
    ...
    # L49: profile_extra убран. Документ (все профили) кэшируется только по
    # пути — re-parse при смене профиля не нужен, выбор профиля делается в
    # load_similarity_thresholds() через config.get_profile(resolve_active_profile()).
)
...
def load_similarity_thresholds() -> SimilarityThresholdsProfile:
    ...
    if active_name != _DEFAULT_PROFILE:
        default_profile = config.get_profile(_DEFAULT_PROFILE)
        profile_fields = (
            "rag_examples_min_score",
            "strategic_memory_min_score",
            "tactical_memory_min_score",
        ...
        if all(getattr(profile, f) == getattr(default_profile, f) for f in profile_fields):
            _logger.warning(
                "similarity_thresholds profile '%s' is identical to 'default' — "
                "it has not been tuned yet. Using default thresholds.",
                active_name,
            )
    return profile
