...
#### Построение JOIN'ов: стратегия пути и containment-валидатор

После того как нужные таблицы и колонки найдены, их надо корректно соединить. Этим занимается `JoinValidator.build_joins` (`custom_tools/text_to_sql/schema_linking/join_validation.py`): собирает рёбра-кандидаты (FK из метаданных; bridge-таблицы — промежуточные таблицы-связки; fallback по суффиксу `*_id`) и строит цепочку JOIN.

По умолчанию используется **greedy**-подключение — простой проход по рёбрам в порядке списка. Опционально доступны две training-free техники (идеи из EcoTable, см. [дизайн-док](../../docs/plans/2026-06-29-text2sql-graph-join-containment-design.md)):

- **Граф-путь (`path_algo: graph`)** — минимальный join-путь как граф-алгоритм (Steiner-tree — минимальное дерево, соединяющее нужные таблицы; KMB-аппроксимация, `join_path.py`). Учитывает «стоимость» ребра (FK дешевле, чем слабое convention-ребро) и осознанно выбирает минимальный путь и правильную bridge-таблицу, тогда как greedy берёт произвольный подходящий вариант. Граф **никогда не хуже greedy**: при любом сбое (число целевых таблиц-терминалов сверх `max_terminals`, не удалось построить связный путь, любое исключение) происходит автоматический откат на greedy с записью в лог. Если таблицы по-настоящему несоединимы, greedy после отката честно вернёт их в `unconnected_tables`.
- **Containment-валидатор (`containment: validate | weight`)** — дешёвая проверка по данным, что джойн реально пересекается: `|A∩B|/|A|` на сэмплах через `db_plugins` (`join_containment.py`). В режиме `validate` convention-рёбра (неуверенные связи по суффиксу `*_id`) с долей пересечения ниже `min_containment` отбрасываются (отсев «правдоподобных, но пустых» связей); в режиме `weight` им повышается стоимость в графе (`min_containment` в этом режиме не применяется). FK/bridge-рёбра не сэмплируются (целостность гарантирована метаданными). Поведение **fail-open**: ошибка БД, таймаут или неопределимость → ребро остаётся, пайплайн не падает.

Стратегия настраивается профилями в `config/text_to_sql/joins.yaml` (`default` — консервативный, `muni_ru` — агрессивный) и/или переменными окружения. Профиль `default` = `greedy`/`off`, поэтому поведение по умолчанию совпадает с прежним.

| Переменная окружения | YAML-ключ | Значения | Назначение |
|----------------------|-----------|----------|-----------|
| `TEXT_TO_SQL_JOINS_PROFILE` | — | имя профиля (`default`/`muni_ru`) | Активный профиль |
| `TEXT_TO_SQL_JOIN_PATH_ALGO` | `path_algo` | `greedy` \| `graph` | Алгоритм построения пути |
| `TEXT_TO_SQL_JOIN_CONTAINMENT` | `containment` | `off` \| `validate` \| `weight` | Режим containment-валидатора |
| `TEXT_TO_SQL_JOIN_MIN_CONTAINMENT` | `min_containment` | float `[0..1]` | Порог отсева ребра в режиме `validate` |
| `TEXT_TO_SQL_JOIN_PATH_MAX_TERMINALS` | `max_terminals` | int `>0` | Лимит целевых таблиц-терминалов для графа (сверх — откат на greedy) |
| `TEXT_TO_SQL_CONTAINMENT_SAMPLE` | `containment_sample` | int `>0` | Размер сэмпла для оценки пересечения |

Значения по умолчанию (профиль `default`, действуют и при включении графа поверх него через env): `max_terminals = 8`, `min_containment = 0.1`, `containment_sample = 1000`.

Приоритет: переменная окружения отдельного ключа > активный профиль. Невалидное значение приводит к ошибке на старте (fail-fast), а не к тихому откату на дефолт.
