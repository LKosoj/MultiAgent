...
Автоматическое обогащение схемы не всегда даёт идеальный результат — иногда описание таблицы или колонки, придуманное LLM, нужно поправить вручную, добавить термин в глоссарий (например, что "выручка" — это колонка `amount` таблицы `orders`) или отклонить неверно определённый факт о схеме. Для этого есть UI-редактор метаданных (`custom_tools/text_to_sql/metadata_editor.py`, действия `text_to_sql.metadata.*`): администратор может отредактировать описания таблиц/колонок, примеры значений, термины глоссария и статус отдельных фактов о схеме, не трогая код и не перезапуская интроспекцию.
...
Стратегия настраивается профилями в `config/text_to_sql/joins.yaml` (`default` — консервативный; более агрессивные настройки задаются доменными подсказками в DSN-профиле) и/или переменными окружения. Профиль `default` = `greedy`/`off`, поэтому поведение по умолчанию совпадает с прежним.

| Переменная окружения | YAML-ключ | Значения | Назначение |
|----------------------|-----------|----------|-----------|
| `TEXT_TO_SQL_JOINS_PROFILE` | — | имя профиля (`default`/...) | Активный профиль |
