...
class CodeInterpreterPlugin():
    ...
    async def aclose(self) -> None:
        """Корректно закрывает подлежащий httpx.AsyncClient во избежание утечки соединений."""
    ...
    def analyze_code_syntax(self, code):
        ...
        Returns:
            dict: {"status": True} при корректном синтаксисе или {"error": <описание>} при ошибке.
        """
        try:
    ...
    async def execute_code(self, code):
        ...
        if isinstance(analyze_result, dict) and analyze_result.get('status'):
...
@tool
def code_generation(prompt: str, session_id: str = None) -> str:
    """Генерирует Python-код на основе текстового описания.

    Args:
        prompt: Текстовое описание для генерации кода
        session_id: Идентификатор сессии (опционально)

    Returns:
        str: Сгенерированный Python-код
    """
    interpreter = CodeInterpreterPlugin()
    code = asyncio.run(interpreter.generate_code(prompt, session_id))
    return code

@tool
def code_execution(code: str, data_path: str = None) -> Dict[str, object]:
    """Выполняет Python-код с безопасными ограничениями.

    Args:
        code: Python-код для выполнения
        data_path: Путь к файлу с данными (опционально)

    Returns:
        str: Результат выполнения кода
    """
    interpreter = CodeInterpreterPlugin()
    session_id = str(uuid.uuid4())[:8]
    result = asyncio.run(interpreter.run_code(data_path, code, session_id))
    return result

@tool
def data_analysis(data_path: str, analysis_prompt: str) -> Dict[str, object]:
    """Выполняет анализ данных с помощью сгенерированного кода.

    Args:
        data_path: Путь к файлу с данными для анализа
        analysis_prompt: Текстовое описание требуемого анализа

    Returns:
        str: Результаты анализа данных
    """
    interpreter = CodeInterpreterPlugin()
    session_id = str(uuid.uuid4())[:8]
    result =  asyncio.run(interpreter.run_code(data_path, analysis_prompt, session_id))
    return result

@tool
def code_interpreter_tool(code_prompt: str, session_id: str = None, data_path: str = None) -> Dict[str, object]:
    """Выполняет Python-код или генерирует код на основе текстового описания задачи.

    Args:
        code_prompt: Текстовое описание задачи или Python код для выполнения
        session_id: Идентификатор сессии (опционально)
        data_path: Путь к файлу с данными (опционально)

    Returns:
        Dict[str, object]: Результат выполнения кода
    """
    interpreter = CodeInterpreterPlugin()
    if not session_id:
        session_id = str(uuid.uuid4())[:8]
    result = asyncio.run(interpreter.run_code(data_path, code_prompt, session_id))
    return result
