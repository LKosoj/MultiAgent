...
class ResourcePool:
    ...
    async def acquire_resources(self, workflow_id: str, 
        ...
        async with self.lock:
            # Очищаем старые API calls (окно 1 минута)
            now = datetime.now()
            ...
            if (self.current_concurrent + required_concurrent > self.max_concurrent or
                ...
                len(self.api_calls_window) + required_api_calls > self.max_api_calls_per_minute):
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "api_calls_per_minute": f"{len(self.api_calls_window)}/{self.max_api_calls_per_minute}",
